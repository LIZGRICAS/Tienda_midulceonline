...
const listProducts = () =>
    fetch('https://apimidulceonline.azurewebsites.net/productos/all').then((response) => response.json()).catch((err) => console.log(err));
...
return fetch(`https://apimidulceonline.azurewebsites.net/productos/destroy/${id}`, { /* backticks para insertar variable */
...
const detailProduct = async (id) => {
    return fetch(`https://apimidulceonline.azurewebsites.net/productos/search/${id}`).then((response) =>
        response.json()
...
fetch(`https://apimidulceonline.azurewebsites.net/productos/update/${_id}`, {
    method: "PUT",
    headers: {
    "Content-Type": "application/json",
    },
